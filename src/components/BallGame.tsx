...
const BallGame = () => {
  ...
  const createNewBall = () => {
    const newBall: Ball = {
      ...
      radius: 30 + Math.random() * 20, // от 30 до 50 пикселей
      createdAt: Date.now(),
      caught: false
    };
  ...
  const drawBalls = (canvasCtx: CanvasRenderingContext2D) => {
    ...
    balls.forEach(ball => {
      ...
      gradient.addColorStop(0.3, 'rgba(255, 255, 150, 0.9)'); // ЯРКО-ЖЕЛТЫЙ
      gradient.addColorStop(0.7, 'rgba(255, 255, 50, 0.5)'); // ЖЕЛТЫЙ
      gradient.addColorStop(1, 'rgba(255, 200, 0, 0)'); // ПРОЗРАЧНЫЙ
      ...
      canvasCtx.beginPath();
      canvasCtx.arc(x, y, ball.radius, 0, 2 * Math.PI);
      canvasCtx.strokeStyle = 'rgba(255, 255, 200, 1)'; // БЕЛЫЙ контур
  ...
  const checkCollisions = (poseLandmarks: any, canvasCtx: CanvasRenderingContext2D) => {
    ...
    // Создаем копию массива для обновления
    let updatedBalls = [...ballsRef.current];
    
    // Проверяем каждый шар на столкновение
    updatedBalls = updatedBalls.map(ball => {
      ...
      if (leftWrist) {
        ...
        // console.log(wristX, wristY, ballX, ballY);
      ...
      if (caught) {
        ...
        // Помечаем шар как пойманный (он будет удален)
        return {
          ...ball,
          caught: true
        };
    ...
    if (caughtAny) {
      ...
      // Создаем новый шар взамен каждого пойманного
      ...
      // Объединяем оставшиеся шары с новыми
